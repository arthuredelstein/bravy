...
const override = (rawFilePath, { both, debug }) => {
  ...
  let contents, filePath, originalPath;
  if (rawFilePath.endsWith('.mojom')) {
    ({ filePath, originalPath } = findOverrideDestinationPath(rawFilePath));
    contents = createMojomContents(filePath, originalPath);
  }
  if (both || rawFilePath.endsWith('.h')) {
    ({ filePath } = findOverrideDestinationPath(rawFileStem + '.h'));
    contents = createHeaderContents(filePath);
  }
  if (both || rawFilePath.endsWith('.cc')) {
    ({ filePath } = findOverrideDestinationPath(rawFileStem + '.cc'));
    contents = createSourceContents(filePath);
  }
  const finalPath = path.join(chromiumSrc, filePath)
  writeNewFile(finalPath, contents, debug);
  gitAdd(finalPath)
};
